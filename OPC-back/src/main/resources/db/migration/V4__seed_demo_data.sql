...
SELECT b.id, p.id,
    -- cantidad: variada por sucursal/producto usando el id para no repetir todo igual
    CASE (b.id + p.id) % 5
    ...
    20,   -- min_stock
    300,  -- max_stock
    p.reference_price * 0.7  -- costo promedio ponderado inicial, estimado a partir del precio de referencia
FROM ma_branches b
