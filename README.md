...
| Inventario | ✅ Completo | Consulta de stock por sucursal (con columna de costo promedio ponderado), registro de ingresos/retiros con validación de stock y recálculo del costo promedio ponderado, alertas de stock bajo/alto |
| Compras | ✅ Completo | Proveedores; órdenes de compra con precio unitario, descuento por ítem en porcentaje y plazo de pago; ciclo de vida de la orden (borrador → enviada al proveedor → recibida / cancelada), edición mientras está en borrador; el listado agrupa las órdenes por estado (recibidas, pendientes por recibir, pendientes por enviar, canceladas); recepción total/parcial que actualiza inventario y recalcula el costo promedio ponderado; histórico de compras filtrable por proveedor, producto y rango de fechas |
| Ventas | ✅ Completo | Clientes, listas de precios (con vigencia por fecha), registro de venta con descuento por línea en porcentaje y validación de stock (el formulario bloquea la confirmación si una línea supera el stock; el backend igual la rechaza), histórico filtrable con el responsable de cada venta, y comprobante por venta (encabezado + ítems + totales) para consulta posterior |
| Transferencias entre sucursales | ✅ Completo | Solicitud, preparación, despacho, recepción completa/parcial con línea de tiempo visual; ante un faltante en la recepción parcial se define el tratamiento (reenvío / ajuste / reclamación), y el reenvío genera automáticamente una transferencia de seguimiento por lo que faltó; cada sucursal solo ve las transferencias en las que participa como origen o destino (el administrador general ve todas); clasificación por prioridad de ruta y reporte de cumplimiento logístico (% a tiempo por sucursal y prioridad) |
