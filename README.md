...
| Alertas y auditoría | ✅ Completo | Notificaciones (`sy_notifications`): cruce de umbral de stock bajo/alto en cada movimiento, faltante de transferencia por ítem y producto sin existencias al crearlo (una por sucursal activa que quede sin stock), con campana en el frontend (polling, marcado de leídas, filtro por tipo). Auditoría (`sy_audit_log`): interceptor genérico sobre eventos de Hibernate que registra cada alta/edición/baja del **catálogo de productos** (quién, cuándo y qué cambió — sección 3.1 del enunciado), con vista de consulta filtrable y diff antes/después para el administrador general; la vista resuelve el nombre y SKU del producto de cada evento (no solo su id), traduce a texto las asociaciones guardadas como id (categoría, unidad base) y muestra los nombres de campo en español. No audita inicios de sesión, ventas ni transferencias (esas tienen su propia trazabilidad: `tr_inventory_movements`, `tr_transfer_events`) |
