...
|---|---|---|---|
| Identidad y acceso (`ma_users`, `ma_roles`, `ma_branches`, `ma_user_branch`) | ✅ Login JWT, `GET /api/auth/me`, CRUD de usuarios y sucursales, `BranchAccessService` (autorización por sucursal) | ✅ Login, rutas protegidas, gestión de usuarios/sucursales | `sy_refresh_tokens` existe en el esquema (ver ADR-003) pero **el endpoint de refresh todavía no está implementado** — hoy solo hay login con access token; la tabla queda lista para cuando se aborde esa tarjeta |
...
| Transferencias (`tr_transfers`, `tr_transfer_items`) | ❌ No implementado | ❌ No implementado | Paquete `opcback.transfers` existe pero vacío; el `MovementType` ya contempla `TRANSFER_IN`/`TRANSFER_OUT` para cuando se aborde |
| Sistema — alertas y auditoría (`sy_notifications`, `sy_audit_log`) | ❌ No implementado | ❌ No implementado | Decisión explícita de dejar Auditoría para el final del backlog (épica casi al cierre del plan back/front) |
