...
| `CATEGORIAS_PRODUCTO` | `ma_categories` | Renombrada + agrega `updated_at`; nombre único (validado en servicio) |
| `UNIDADES_MEDIDA` | `ma_units` | Renombrada; agrega `active` (borrado lógico, `V14`) |
...
|---|---|---|
| `id` | BIGINT PK | |
| `name` | VARCHAR(100) | Único (validado en `CategoryService`, sin índice — ver `V14`) |
| `description` | VARCHAR(255) | |
| `active` | BOOLEAN | Borrado lógico; el físico (`DELETE`) se bloquea si hay productos en la categoría |
| `updated_at` | DATETIME | |
...
|---|---|---|
| `id` | BIGINT PK | |
| `name` | VARCHAR(50) | Único (validado en `UnitService`, sin índice — ver `V14`) |
| `abbreviation` | VARCHAR(10) | Único (validado en `UnitService`) |
| `active` | BOOLEAN | Borrado lógico (`V14`); el físico se bloquea si algún producto la usa como unidad base o alternativa |
...
|---|---|---|
| `id` | BIGINT PK | |
| `type` | ENUM(`LOW_STOCK`, `HIGH_STOCK`, `TRANSFER_SHORTAGE`, `OUT_OF_STOCK`, `TRANSFER_PENDING`, `PURCHASE_ORDER_PENDING`) | `OUT_OF_STOCK` (`V15`): stock en cero. `TRANSFER_PENDING` / `PURCHASE_ORDER_PENDING` (`V17`): notificaciones de flujo de trabajo (una transferencia u orden que espera una acción), solo visibles para el gerente de la sucursal y el administrador general |
| `branch_id` | BIGINT FK → `ma_branches` | |
| `product_id` | BIGINT FK → `ma_products`, NULL | |
| `reference_id` | BIGINT NULL | `V16` — id de la entidad que originó la notificación cuando no es un producto: la transferencia de un `TRANSFER_SHORTAGE` / `TRANSFER_PENDING`, o la orden de un `PURCHASE_ORDER_PENDING`. Validado en el service layer, no por FK |
| `message` | VARCHAR(255) | |
...
- [x] Migraciones versionadas con Flyway — `OPC-back/src/main/resources/db/migration/V1` a `V17` (mismo contenido que los scripts anteriores; ver ADR-007 en `requirements/Decisiones_Arquitectura.md` para la justificación).
...
| Transferencias (`tr_transfers`, `tr_transfer_items`, `tr_transfer_events`) | ✅ Solicitud (rechaza si la sucursal de origen no tiene existencias suficientes del producto — el inventario es compartido), preparación, despacho, recepción completa/parcial, eventos automáticos; tratamiento del faltante de una recepción parcial (`POST /transfers/{id}/resolve-shortage`: reenvío / ajuste / reclamación — el reenvío crea una transferencia de seguimiento `REQUESTED` por lo faltante); filtro/clasificación por `route_priority` y reporte de cumplimiento logístico (`% a tiempo` por sucursal origen y prioridad). **Solicitar** y consultar es de cualquier rol asignado a la sucursal; **preparar, despachar, recibir, tratar el faltante y clasificar la ruta** exigen `BranchAccessService.assertCanManage` (gerente de la sucursal o ADMIN_GENERAL — el operador no). Las fechas del envío se validan en cadena: la estimada de despacho (fijada al preparar) no puede ser anterior a `request_date`, y la estimada de llegada (fijada al despachar) no puede ser anterior a la de despacho | ✅ Panel de transferencias activas (auto-refresco cada 20s sin recargar la página) con filtro por prioridad de ruta y columna de prioridad, solicitud, preparar/despachar, recibir completa/parcial con línea de tiempo visual del historial completo, formulario de tratamiento del faltante en el detalle, control para clasificar la ruta por prioridad (sucursal origen), tabla de tiempos estimados vs. reales (despacho y llegada, con desviación en días) y visualización del reporte de cumplimiento (gráfica de barras + tabla). El operador de inventario no ve los formularios de gestión, solo consulta y solicita | Con tests unitarios (`TransferServiceTest`). Lectura acotada por sucursal: el listado y el detalle solo muestran transferencias donde alguna sucursal del usuario es origen o destino (ADMIN_GENERAL ve todas) — mismo criterio de lectura que el listado de notificaciones. `route_priority` se fija con `PATCH /transfers/{id}/route-priority` (nivel gestión, bloqueado una vez la transferencia ya llegó); `estimated_dispatch_date` se registra al preparar el envío (sección 3.5) |
| Dashboard / KPIs operativos (agregación sobre `tr_sales`, `tr_inventory_movements`, `tr_transfers`, `tr_inventory`) | ✅ 5 endpoints en `opcback.dashboard`: ventas del mes vs. 3 anteriores (serie temporal por sucursal); rotación de inventario ordenable (`order=ASC/DESC`, con rango de fechas) que lista **todos** los productos activos con inventario en la sucursal — los de rotación 0 son la "baja demanda"; impacto proyectado de transferencias activas **con desglose por estado**; productos por reabastecer (reusa `InventoryService.listAlertsByBranch`, sin duplicar el criterio de alerta); comparativa entre sucursales (`ADMIN_GENERAL` únicamente) | ✅ Página "Panel" con las 5 secciones del PDF usando Recharts, selector de sucursal, conmutador alta/baja demanda + fechas en el card de rotación, desglose por estado en el de transferencias, y la comparativa visible solo para administrador general. El ítem "Panel" del riel y la ruta `/dashboard` están ocultos/bloqueados para el operador de inventario (`DASHBOARD_ROLES`; su inicio es `/inventario`) | No es un dominio con tablas propias — agrega sobre datos que ya escriben Ventas, Inventario y Transferencias, de solo lectura. Con `DashboardServiceTest` |
| Sistema — alertas y auditoría (`sy_notifications`, `sy_audit_log`) | ✅ `NotificationService` (único escritor de `sy_notifications`) sigue un **modelo de reconciliación**: `sy_notifications` refleja el estado pendiente actual, no un histórico. `reconcileStockNotification` es el criterio único — dado un producto en una sucursal decide qué notificación de stock debe existir (`LOW_STOCK` / `HIGH_STOCK` / `OUT_OF_STOCK`) y crea, reemplaza o **borra** en consecuencia. La llaman con el mismo criterio (a) el disparo instantáneo: cada movimiento, cada cambio de umbral y el alta de producto (una `OUT_OF_STOCK` por sucursal activa sin existencias); y (b) el chequeo programado `NotificationReconciliationJob` (`@Scheduled`, cron configurable `notifications.reconciliation.cron`, por defecto cada 2 h entre las 7:00 y las 19:00, zona Bogotá), que además vuelve a `PENDING` las notificaciones leídas cuyo problema sigue vigente. `TRANSFER_SHORTAGE` guarda el id de la transferencia en `reference_id` (`V16`) y se borra al tratar el faltante (`resolveShortage` → `clearTransferShortage`). El mismo modelo cubre las **notificaciones de flujo de trabajo** (`V17`): `reconcileTransferNotification` / `reconcilePurchaseOrderNotification` mantienen una notificación por transferencia (`TRANSFER_PENDING`) y por orden de compra (`PURCHASE_ORDER_PENDING`) mientras esperan una acción — el `branch_id` sigue a la sucursal que debe actuar (origen mientras se prepara/despacha, destino en tránsito y para tratar el faltante) — y la borran al cerrarse; las llaman `TransferService` / `PurchaseOrderService` / `PurchaseReceiptService` tras cada transición y el chequeo programado. `NotificationService.list` oculta estas dos al `INVENTORY_OPERATOR` (`BranchAccessService.isBranchManager`): solo las ven el gerente de la sucursal y el administrador general. Canal `IN_APP` (el `EMAIL` del ENUM queda reservado, sin envío real). `AuditEntityListener` (registrado en el `EventListenerRegistry` de Hibernate por `AuditListenerConfig`): audita `POST_INSERT`/`UPDATE`/`DELETE` de toda entidad que implemente `Auditable` — hoy solo `Product` — sin que ningún servicio de negocio lo sepa. Escritura por JDBC directo, lectura por JPA; `AuditLogService` resuelve el nombre + SKU del producto de cada evento y traduce a texto las asociaciones que el listener guarda como id (`category` → nombre de categoría, `baseUnit` → «Nombre (abreviatura)»), todo en 3 consultas por lote, con fallback a los valores del propio registro | ✅ Campana de notificaciones (polling 20s, filtro por tipo; al pulsar una notificación se marca leída y se navega a donde se atiende — `notificationLink`: stock → Inventario de esa sucursal con el producto filtrado, faltante / transferencia en curso → detalle de la transferencia, orden de compra en curso → detalle de la orden) y vista de consulta de auditoría de productos (`GET /api/auditoria`, solo `ADMIN_GENERAL`) con filtros (id de producto, responsable, fechas), columna con el nombre del producto, nombres de campo en español y diff antes/después | `V6` deja `sy_audit_log.user_id` nullable. `V9` retira `LOGIN` del ENUM y limpia login + transferencias. `V10` deja la auditoría solo en productos (borra el resto) |
