...
|---|---|---|
| `id` | BIGINT PK | |
| `type` | ENUM(`LOW_STOCK`, `HIGH_STOCK`, `TRANSFER_SHORTAGE`, `OUT_OF_STOCK`) | `OUT_OF_STOCK` (`V15`): stock en cero |
| `branch_id` | BIGINT FK → `ma_branches` | |
| `product_id` | BIGINT FK → `ma_products`, NULL | |
| `reference_id` | BIGINT NULL | `V16` — id de la entidad que originó la notificación cuando no es un producto (hoy solo la transferencia de un `TRANSFER_SHORTAGE`). Validado en el service layer, no por FK |
| `message` | VARCHAR(255) | |
...
- [x] Migraciones versionadas con Flyway — `OPC-back/src/main/resources/db/migration/V1` a `V16` (mismo contenido que los scripts anteriores; ver ADR-007 en `requirements/Decisiones_Arquitectura.md` para la justificación).
