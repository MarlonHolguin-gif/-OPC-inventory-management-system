...
| Múltiples unidades de medida por producto | Resuelto vía `ma_units` + `ma_product_units` (factor de conversión, unidad de compra vs. unidad de venta). *(Se eliminó en `V11` y se restauró en `V12`; en `V13` se cableó de verdad — cada línea de orden de compra y de venta lleva su `unit_id` y el backend convierte a unidad base para el inventario y el costo.)* |
...
2. Rotación de inventario, alta/baja demanda → `SUM(quantity)` de `tr_inventory_movements` (tipo `SALE`) por producto en un rango de fechas, **sobre todos los productos activos con inventario en la sucursal** (los de suma 0 son la "baja demanda"); el frontend ordena ascendente o descendente
3. Transferencias activas y su impacto en inventario → transferencias con `status NOT IN (FULLY_RECEIVED, CANCELLED)`: conteo como origen/destino, **desglose por estado** y proyección de entradas/salidas por producto
...
- **`ma_product_units` + `ma_units` + `ma_products.base_unit_id`**: resuelve exactamente el requisito de "múltiples unidades de medida por producto" (ej. comprar en cajas, vender en unidades) con `conversion_factor` y flags `is_purchase_unit`/`is_sale_unit`. *(Se eliminó en `V11` y se restauró en `V12`.)*
...
| **Tabla `sy_audit_log`** | Requerida por la funcionalidad "Auditoría y trazabilidad" elegida. Log de altas/ediciones/bajas con `old_values`/`new_values`, distinto de `tr_inventory_movements` que es específico de stock. Columnas: `id, entity VARCHAR(60), entity_id BIGINT, action ENUM(CREATE, UPDATE, DELETE), user_id FK NULL, old_values JSON NULL, new_values JSON NULL, event_date DATETIME`. (En la implementación se acotó el alcance a **solo el catálogo de productos** — que es lo que pide literalmente la sección 3.1: "registro de cada acción sobre el inventario: quién, cuándo y por qué". Inventario y transferencias tienen su propia trazabilidad; ver `V9`/`V10`.) |
...
| Todas las demás (`tr_inventory_movements`, `tr_purchase_orders`, `tr_purchase_order_items`, `tr_purchase_receipts`, `tr_purchase_receipt_items`, `tr_sale_items`, `tr_transfer_items`, `tr_transfer_events`, `ma_units`, `ma_product_units`, `ma_price_list_items`) | **Mantener** | Ya cubren correctamente su requisito correspondiente, sin cambios (`ma_product_units` se eliminó en `V11` y se restauró en `V12`) |
...
- [x] Datos mínimos de demostración (roles, sucursales, usuarios, catálogo, inventario) — ver sección "Datos de demostración" en `README.md`.
