...
| **Interceptor / aspecto genérico (auditoría)** | `OPC-back/src/main/java/opcback/system/audit/service/AuditEntityListener.java` — un `PostInsert/PostUpdate/PostDeleteEventListener` de Hibernate registrado una sola vez en el `EventListenerRegistry` (`AuditListenerConfig`) | El requisito de auditoría exige "un interceptor/aspecto genérico, no llamadas dispersas". Un único listener de bajo nivel intercepta todo cambio sobre cualquier entidad que implemente `Auditable`; ningún servicio de negocio (`ProductService`, `UserService`, `TransferService`…) llama a la auditoría ni sabe que existe. Se usa el evento de Hibernate (no `@PreUpdate`/`@PostUpdate` de JPA) porque solo aquel trae el estado *anterior* de la fila junto al nuevo |
